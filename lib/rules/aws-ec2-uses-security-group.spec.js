...
describe('aws-ec2-uses-security-group', function() {
  ...
  it("Returns 'success' when the EC2 instance has one of the security groups", function *() {
    const provider = {};
    const context = {config : ['sg-fc423486','sg-fbcd0182'], instance, provider};
    const result = yield rule.validate( context );
    expect(result.valid).toBe('success');
  });
  it("Returns 'fail' when the EC2 instance is missing both security groups", function *() {
    const provider = {};
