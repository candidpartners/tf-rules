'use strict';
const co = require('co')
const debug = require('debug')('tfrules/aws-security-group-allow-outbound');
...
AWSSecurityGroupAllowOutbound.validate = function *( context ) {
  ...
  if( context.config ) {
    ...
    _.map(context.instance.egress, egress => cidrComparison(context.config, egress, context.instance.name))
