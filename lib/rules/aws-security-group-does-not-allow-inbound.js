    ...
    if ( ! _.isEmpty(insecureGroups[context.instance.name]) ){
      ...
      _.map(insecureGroups[context.instance.name], group => {
        if ( group.cidr && ! group.ports ) {
          ...
          message.push('CIDR Block ' + group.cidr + ' in security group ' + context.instance.name + ' should not be taking inbound traffic on port(s): ' + context.config.port + ' from ips: ' + context.config.cidr)
        }
      })
      result = {
        valid : 'fail',
